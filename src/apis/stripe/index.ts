      ...
      try {
        if(session.metadata.email) {
         ...
         if(user){
          ...
          const order = new OrderModel({user:null ,amount: session.amount_total / 100, products: JSON.parse(session.metadata.products),guest:session.customer_details.email})
      await order.save()
